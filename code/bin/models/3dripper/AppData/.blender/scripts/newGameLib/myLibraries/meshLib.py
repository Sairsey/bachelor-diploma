	...
	def set(self):
		...
		for i,mesh in enumerate(self.meshList):
			if len(mesh.vertPosList)>0:
				...
				file.write(str(i)+':'+mesh.title+':'+MeshNames[i]+'\n')
				mesh.object.name = mesh.title
				#for j,mat in enumerate(mesh.matList):
					#if mesh.SPLIT==False:
...
class Mesh():
	...
	def __init__(self,Name1,Name2):
		...
		self.name=str(MeshNum)+'-_'+Name1
		self.title=str(MeshNum)+'-_'+Name2
		MeshNames.append(Name1)
		MeshNum=MeshNum+1
	...
	def addMat(self,mat,mesh,matID):
		...
		mat.name=str(MatNum)+'-_'+mesh.name
	...
	def buildMesh(self,mesh,mat,meshID):
		...
		if len(mesh.vertPosList)>0:
			...
			if len(mesh.triangleList)>0:	
				if len(mesh.vertUVLists)>0:
					for uvList in mesh.vertUVLists:
						mesh.vertUVList = uvList
	...
	def draw(self,id=None): 
			...
			if self.SPLIT==False:
				...
				if len(self.triangleList)>0:	
					i = 0
					for uvList in self.vertUVLists:
						if (len(uvList) > 0):
							self.mesh.addUVLayer("UV" + str(i))
							self.mesh.activeUVLayer = "UV" + str(i)
							self.mesh.renderUVLayer = "UV" + str(i)
							self.vertUVList = uvList
