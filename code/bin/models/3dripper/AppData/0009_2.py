...
def getAnimation(ys,A,n):
	...
	if Channels:
		...
		action.name=Name
		new=New("BinFiles" + os.sep + Name.replace('"','').replace('|','')+'.action','wb',Blender.sys).open()
		...
		if Float32Array:
			...
			if Encoding != '"varint"':
				...
				if path:
					...
					with open("./" + str(modelID) + ".dat", "a") as modelData:
						modelID += 1
						for line in bytes:
...
def osgAnimationRigGeometry(ys,parentNewNode,Name1,Name2):
	print "* * * * * * * * * * * * * * * * * * * * * *"
	print Name1
	print Name2
	mesh=Mesh(Name1,Name2)
	mesh.vertexArray=[]
	mesh.indiceArray=[]
	mesh.TexCoord0Array=[]
	mesh.TexCoord1Array=[]
					...
					for child in child.children:
						...
						if '"VertexAttributeList"' in child.node.header:
							for child in child.children:
								...
								if '"Normal"' in child.node.header:
									mesh.normalArray=VertexNormal(ys,child.node)
...
def osgGeometry(ys,parentNewNode,Name1,Name2):
	print "* * * * * * * * * * * * * * * * * * * * * *"
	print Name1
	print Name2
	mesh=Mesh(Name1,Name2)
...
def osgAnimationMorphGeometry(ys,parentNewNode,Name1,Name2):
	print "* * * * * * * * * * * * * * * * * * * * * *"
	print Name1
	print Name2
	mesh=Mesh(Name1,Name2)
...
def drawMesh(ys,mesh):	
	...
	try:
		if len(mesh.normalArray) == 1:
			if mesh.normalArray[0][1] == '"varint"':
				bytes = mesh.normalArray[0][0]
				ItemSize = mesh.normalArray[0][2]
				eps = float(mesh.atributes["epsilon"])
				nphi = float(mesh.atributes["nphi"])
				size = len(bytes) / ItemSize
				ints = bytes
				global modelID
	...
	if len(mesh.TexCoord0Array)==1:
		if mesh.TexCoord0Array[0][1]=='"varint"':
			if mode:
				...
				mesh.vertUVLists.append(uvList)
		else:
			list=mesh.TexCoord0Array[0][0]
			mesh.vertUVLists=list
	...
	for child in parentNewNode.children:
		...
		try:
			...
			for C2 in C.children:
				if '"StateSet"' in C2.node.header:
					#print 'FOUND FOUND FOUND FOUND '
					C3 = C2.children[0]
					if '"Name"' in C3.node.header:
						values1=ys.values(C3.node.header,':')
						Name4 = ys.getValue(values1,'"Name"','""')
						print 'Name4 = '+Name4
					C3 = C3.children[0]
					if '"Name"' in C3.node.header:
						values1=ys.values(C3.node.header,':')
		...
		try:
			if '"Name"' in child.node.header:
				values1=ys.values(child.node.header,':')
				Name3 = ys.getValue(values1,'"Name"','""')
				print 'Name3 = '+Name3
		except:
				#print 'Err Name3'
				Name3=Name3
		...
		for child in child.children:
			...
			try:
				C = child.children[0]
				if '"Name"' in C.node.header:
					values1=ys.values(C.node.header,':')
			...
			try:
				...
				if '"Name"' in C.node.header:
					values1=ys.values(C.node.header,':')
					Name1 = ys.getValue(values1,'"Name"','""')
					#print 'Name1 = '+Name1
			except:
				#print 'Err Name1'
				Name1=Name1
			...
			if Name1 == '':
				try:
					C = child.children[0]
					if '"StateSet"' in C.node.header:
						C2 = C.children[0]
						if '"Name"' in C2.node.header:
							values1=ys.values(C2.node.header,':')
							Name1 = ys.getValue(values1,'"Name"','""')
							#print 'Name1 = '+Name1
						C2 = C2.children[0]
						if '"Name"' in C2.node.header:
			...
			if Name1 == '':
				try:
					C = child.children[0]
					if '"StateSet"' in C.node.header:
						#print 'Trying to find UniqueID'
						C2 = C.children[0]
						if '"UniqueID"' in C2.node.data:
							#print 'Found !! ' + C2.node.data
							CArray = C2.node.data.split('UniqueID":')
							Name1 = "UniqueID_"+CArray[1]
							#print 'Name1 = '+Name1
			...
			if '"osg.Geometry"' in child.node.header:
				mesh=osgGeometry(ys,child,Name1,Name2)
				mesh.parentBone=parentBone
				model.meshList.append(mesh)
			if '"osgAnimation.RigGeometry"' in child.node.header:
				mesh=osgAnimationRigGeometry(ys,child,Name1,Name2)
				mesh.parentBone=parentBone
				model.meshList.append(mesh)
			if '"osgAnimation.MorphGeometry"' in child.node.header:
				mesh=osgAnimationMorphGeometry(ys,child,Name1,Name2)
				mesh.parentBone=parentBone
				model.meshList.append(mesh)
...
#def sss():	
	...
	if result10==1:
		for i1,mesh in enumerate(model.meshList):
			#print i1,'vert:',len(mesh.vertPosList),'col:',len(mesh.vertColList),'uv:',len(mesh.vertUVLists),'indice:',len(mesh.indiceList),'mat:',len(mesh.matList),'skin:',len(mesh.skinList)#,matName
...
def Parser(path):
	...
	filename=path
	sysDir=Blender.sys.dirname(filename)
	os.system("cls")
	ext=filename.split('.')[-1].lower()
	if ext=='gz':
		osg=sysDir+os.sep+'file.osgjs'
		if os.path.exists(osg)==False:
			cmd=Cmd()
			cmd.input=filename
			cmd.ZIP=True
			cmd.run()
			model=sysDir+os.sep+'model_file.bin.gz'
	...
	if ext=='txt':
		osgParser(filename)
	if ext in ['htm','html']:
		osg=sysDir+os.sep+'file.osgjs.gz'
		...
		if os.path.exists(model)==True:
			...
			model=sysDir+os.sep+'model_file.bin'
		if os.path.exists(model):
			...
			osgPath=sysDir+os.sep+'file.osgjs'
