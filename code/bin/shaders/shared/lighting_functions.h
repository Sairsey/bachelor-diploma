...
bool CalcLight(in uint LightIndex, in float3 Position, out float3 DirectionToLight, out float3 LightColor)
{
  ...
  if (light.Color.x == -1 && light.Color.y == -1 && light.Color.z == -1)
      LightColor = shadowMultiplier.xxx;
  else
  {
...
float4 ShadeCookTorrance(float3 Normal, float3 Position, float2 uv, GDRGPUMaterial material, PBRParams Params)
{
    ...
    float IBLShadow = 0;
    int IBLShadowCount = 0;
    float IBLMinimum = 0.3;
    ...
    for (uint i = 0; i < globals.LightsAmount; i++)
    {
        ...
        if (LightsPool[i].Color.x == -1 && LightsPool[i].Color.y == -1 && LightsPool[i].Color.z == -1)
        {
            IBLShadow += LColor.x;
            IBLShadowCount++;
            continue;
    ...
    if (globals.IsIBL && env.PrefilteredCubemapIndex != NONE_INDEX)
    {
        ...
        if (IBLShadowCount != 0)
            Ambient *= clamp(IBLShadow / IBLShadowCount, IBLMinimum, 1);
        resultColor += Ambient;
    }
